...
import {constantRouterMap,asyncRouterMap,whiteList} from './routes'
...
router.beforeEach((to, from, next) => {
  NProgress.start();
  if (to.path == '/login') {console.log('/login')
    store.dispatch('LogOut');
    next();
  }else{console.log('router',router);
    if (store.getters.token) {console.log('store.getters.token',store.getters.token)
